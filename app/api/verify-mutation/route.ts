  ...
  try {
    ...
    if (!ticket) {
      return NextResponse.json(
        {
          error:
            "No active session ticket found. Middleware might be bypassed or blocked.",
        },
        { status: 401 },
      );
    }
    ...
    return NextResponse.json({
      ...
      mutationMap,
      verificationNote:
        "Session selectors are randomized and stable per ticket, which validates active polymorphic shielding.",
    });
  } catch {
    return NextResponse.json(
