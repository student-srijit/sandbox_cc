...
async function probeBackend(): Promise<{ ok: boolean; latencyMs: number | null }> {
...
async function probePublicStats(): Promise<{ activeSessions: number | null }> {
  try {
    ...
    return {
      activeSessions: typeof data.active_sessions === 'number' ? data.active_sessions : null,
    }
  } catch {
...
async function probeRpc(url: string): Promise<RpcProbe> {
  ...
  try {
    ...
    const blockNumber = typeof blockHex === 'string' ? parseInt(blockHex, 16) : NaN
...
export async function GET() {
  const [backend, publicStats, sepoliaRpc, celoRpc] = await Promise.all([
    ...
    probeRpc(process.env.CELO_SEPOLIA_RPC_URL || process.env.CELO_ALFAJORES_RPC_URL || 'https://forno.celo-sepolia.celo-testnet.org'),
  ])
