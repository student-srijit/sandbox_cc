...
export default function VaultPage() {
    ...
    const activeContracts = useMemo(() => getContractsForChain(activeChain), [activeChain])
    ...
    const ensureWriteAccess = async () => {
        ...
        if (!isContractsConfigured(chainContracts)) {
            throw new Error(`Contracts are not configured for ${CHAIN_LABEL[supported]}`)
        }
    ...
    const loadLivePricing = useCallback(async () => {
        ...
        try {
            ...
            const report = new Contract(contracts.reportAccessNFT, REPORT_ACCESS_NFT_ABI, provider)
            const enterprise = new Contract(contracts.enterpriseLicenseNFT, ENTERPRISE_LICENSE_NFT_ABI, provider)
            ...
            setPrices({
                ...
                reportStable: ethers.formatUnits(reportPricing.stablePrice as bigint, 6),
    ...
    const handleBuyReport = async () => {
        ...
        try {
            ...
            const reportId = ethers.id(reportKey.trim())
            const report = new Contract(chainContracts.reportAccessNFT, REPORT_ACCESS_NFT_ABI, signer)
            const stable = new Contract(chainContracts.stableToken, ERC20_ABI, signer)
            ...
            if (paymentMode === 'native') {
                ...
                const allowance = await stable.allowance(buyer, chainContracts.reportAccessNFT)
                if ((allowance as bigint) < (pricing.stablePrice as bigint)) {
                    const approveTx = await stable.approve(chainContracts.reportAccessNFT, pricing.stablePrice)
    ...
    const handleBuyLicense = async () => {
        ...
        try {
            ...
            const buyer = await signer.getAddress()
            const planId = ethers.id(planKey.trim())
            const enterprise = new Contract(chainContracts.enterpriseLicenseNFT, ENTERPRISE_LICENSE_NFT_ABI, signer)
            const stable = new Contract(chainContracts.stableToken, ERC20_ABI, signer)
            ...
            if (paymentMode === 'native') {
                ...
                const allowance = await stable.allowance(buyer, chainContracts.enterpriseLicenseNFT)
                if ((allowance as bigint) < (plan.stablePrice as bigint)) {
                    const approveTx = await stable.approve(chainContracts.enterpriseLicenseNFT, plan.stablePrice)
                    await awaitTx('Stable approval', chain, approveTx)
                }
    ...
    const handleCreateBounty = async () => {
        ...
        try {
            ...
            const { signer, chain, chainContracts } = await ensureWriteAccess()
            const escrow = new Contract(chainContracts.bountyEscrow, BOUNTY_ESCROW_ABI, signer)
            const stable = new Contract(chainContracts.stableToken, ERC20_ABI, signer)
            ...
            if (paymentMode === 'native') {
                ...
                const allowance = await stable.allowance(creator, chainContracts.bountyEscrow)
                if ((allowance as bigint) < tokenAmount) {
                    const approveTx = await stable.approve(chainContracts.bountyEscrow, tokenAmount)
                    await awaitTx('Stable approval', chain, approveTx)
                }

                const tx = await escrow.createTokenBounty(chainContracts.stableToken, tokenAmount, deadline)
                await awaitTx('Bounty creation', chain, tx)
            }
        } catch (error) {
            showNotification(extractErrorMessage(error))
                    ...
                    <div className="max-w-4xl w-full space-y-6">
                        ...
                        {txState && (
                            <div className="border border-[#00FFD1]/40 bg-[#051116] px-4 py-3 text-xs tracking-widest">
                                <span className="text-[#00FFD1]">TX:</span> {txState.label} on {CHAIN_LABEL[txState.chain]} 
                                <a
                                    className="text-[var(--accent-magenta)] ml-2 underline"
                                    href={`${SUPPORTED_CHAINS[txState.chain].blockExplorerUrls[0]}/tx/${txState.hash}`}
                                    target="_blank"
                                    rel="noreferrer"
                                >
                        ...
                        <div className="text-center mb-12">
                            ...
                            {!isActive ? (
                                <div className="mt-6 flex flex-col items-center gap-3">
                                    <p className="text-[#888] text-xs tracking-widest uppercase">Wallet not connected</p>
                                    <button
                                        onClick={() => connectWallet()}
                                        className="px-8 py-3 text-sm tracking-[0.25em] uppercase font-bold border-2 border-[#00FFD1] text-[#00FFD1] bg-[#00FFD1]/10 hover:bg-[#00FFD1] hover:text-black transition-all shadow-[0_0_20px_rgba(0,255,209,0.2)]"
                                    >
                                        CONNECT WALLET
                                    </button>
                                </div>
                                ...
                                <>
                                    <p className="text-[#00FFD1] mt-4 uppercase tracking-[0.2em]">
                                        ACTIVE CHAIN: {CHAIN_LABEL[activeChain]} | WALLET: {balance || '0.0000'}
                                    </p>
                                    {isActive && chainId !== null && chainId !== preferredChainId && (
                                        <p className="text-[#FF8A00] mt-1 text-xs tracking-widest uppercase">
                                            You&apos;re on {CHAIN_LABEL[asSupportedChain(chainId) ?? DEFAULT_CHAIN_ID] ?? `chain ${chainId}`} &mdash; click SWITCH IN WALLET to move to {CHAIN_LABEL[preferredChainId]}
                                        </p>
                                    )}
                                </>
                            )}
                            <p className="text-[#888] mt-2 text-xs tracking-widest uppercase">
                                Real contract mode: report purchase, enterprise license, and bounty escrow
                            </p>
                        </div>

                        <div className="bg-[#0b0c10]/80 border border-[#222] p-4 flex flex-wrap items-center gap-3">
                            <span className="text-xs tracking-widest text-[#888] uppercase mr-2">Preferred Chain</span>
                            <button
                                onClick={() => setPreferredChainId(11142220)}
                                className={`px-3 py-2 text-xs tracking-widest border ${preferredChainId === 11142220 ? 'border-[#00FFD1] text-[#00FFD1] bg-[#00FFD1]/10' : 'border-[#333] text-[#aaa]'}`}
                            >
                                CELO SEPOLIA
                            </button>
                            <button
                                onClick={() => setPreferredChainId(11155111)}
                                className={`px-3 py-2 text-xs tracking-widest border ${preferredChainId === 11155111 ? 'border-[#00FFD1] text-[#00FFD1] bg-[#00FFD1]/10' : 'border-[#333] text-[#aaa]'}`}
                            >
                                SEPOLIA
                            </button>
                            <button
                                onClick={async () => {
                                    ...
                                    const ok = await switchChain(preferredChainId)
                                    if (!ok) showNotification('Chain switch failed — confirm in MetaMask')
                                }}
                                disabled={isBusy}
                                className="px-3 py-2 text-xs tracking-widest border border-[var(--accent-magenta)] text-[var(--accent-magenta)] hover:text-white disabled:opacity-40"
                            >
                                SWITCH IN WALLET
                            </button>
                            {!hasContractConfig && (
                                <span className="text-[#FF8A00] text-[10px] tracking-widest uppercase">
                                    Contract addresses missing for {CHAIN_LABEL[activeChain]} | run deploy + sync env
                                </span>
                            )}
                        </div>
                        ...
                        <div className="bg-[#0b0c10]/80 backdrop-blur-xl border border-[#222] p-10 relative overflow-hidden shadow-[0_0_50px_rgba(0,255,209,0.05)]">
                            ...
                            <div className="flex flex-wrap items-center justify-between mb-6 border-b border-[#222] pb-6 gap-3">
                                <div>
                                    <h3 className="text-xl text-white font-bold tracking-widest uppercase">Commerce Controls</h3>
                                    <p className="text-xs text-[#666] mt-1">Choose payment mode once, then execute report/license/bounty actions.</p>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => setPaymentMode('native')}
                                        className={`px-3 py-1 text-xs tracking-widest border ${paymentMode === 'native' ? 'border-[#00FFD1] text-[#00FFD1]' : 'border-[#333] text-[#999]'}`}
                                    >
                                        NATIVE
                                    </button>
                                    <button
                                        onClick={() => setPaymentMode('stable')}
                                        className={`px-3 py-1 text-xs tracking-widest border ${paymentMode === 'stable' ? 'border-[#00FFD1] text-[#00FFD1]' : 'border-[#333] text-[#999]'}`}
                                    >
                                        {prices.stableSymbol}
                                    </button>
                                </div>
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <div className="border border-[#222] p-5 space-y-3">
                                    <h4 className="text-white tracking-widest uppercase text-sm">Buy Threat Report NFT</h4>
                                    <input
                                        value={reportKey}
                                    ...
                                    <p className="text-[11px] text-[#888]">
                                        Price: {paymentMode === 'native' ? `${prices.reportNative} Native` : `${prices.reportStable} ${prices.stableSymbol}`}
                                        {' | '}
                                ...
                                <div className="border border-[#222] p-5 space-y-3">
                                    <h4 className="text-white tracking-widest uppercase text-sm">Buy Enterprise License</h4>
                                    <input
                                        value={planKey}
                                    ...
                                    />
                                    <p className="text-[11px] text-[#888]">
                                        Price: {paymentMode === 'native' ? `${prices.planNative} Native` : `${prices.planStable} ${prices.stableSymbol}`}
                                        {' | '}
                                ...
                                <div className="border border-[#222] p-5 space-y-3 md:col-span-2">
                                    <h4 className="text-white tracking-widest uppercase text-sm">Create Bounty Escrow</h4>
                                    <div className="grid md:grid-cols-2 gap-3">
                                        <input
                                            value={bountyAmount}
                                            onChange={(e) => setBountyAmount(e.target.value)}
                                            className="w-full bg-black border border-[#333] px-3 py-2 text-sm"
                                            placeholder={`amount (${paymentMode === 'native' ? 'native' : prices.stableSymbol})`}
                                        />
                                        <input
                                            value={deadlineHours}
                                    ...
                                    <p className="text-[10px] text-[#666]">
                                        Explorer: <a className="underline" href={explorerBase} target="_blank" rel="noreferrer">{explorerBase}</a>
                                    </p>
                                </div>
                            </div>
                        </div>
