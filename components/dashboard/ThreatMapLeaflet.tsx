...
function pinFromLog(log: ThreatMapLog): ThreatPin | null {
  ...
  const severity: ThreatPin["severity"] =
    isProxy || isHosting
      ? "critical"
  ...
  return {
    ...
    confidence:
      typeof log.classification?.confidence === "number"
      ...
      <div className="wr-panel-header border-b border-[#113625]">
        ...
        <div className="flex items-center gap-4 text-[9px]">
          <span className="text-[#FF3030]">
            <span className="text-[13px] font-bold">{totalAttacks}</span>{" "}
            ATTACKS
          </span>
        ...
        <div className="flex items-center gap-1">
          {(Object.keys(MAP_STYLES) as MapStyleKey[]).map((key) => (
            <button
              ...
              className={`px-2 py-1 text-[9px] tracking-[0.18em] border transition-colors ${
                mapStyle === key
              ...
              >
                <Popup>
                  <div className="text-[12px] leading-5">
                    <div>
                      <strong>Threat:</strong> {pin.id}
                    </div>
                    <div>
                      <strong>Attack Type:</strong> {pin.attackType}
                    </div>
                    <div>
                      <strong>Source:</strong> {pin.city}, {pin.country}
                    </div>
                    <div>
                      <strong>IP:</strong> {pin.ip}
                    </div>
                    <div>
        ...
        {pins.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="bg-[#020a07]/90 border border-[#1f3e34] px-6 py-4 text-center">
              <div className="text-[11px] tracking-[0.2em] text-[#95aea6] uppercase">
                No Real Geo Signals Yet
              </div>
              <div className="text-[10px] text-[#6d817a] mt-1">
                Waiting for incoming threats with resolved latitude and
                longitude.
              </div>
            </div>
          </div>
