    ...
    async function pullHealth() {
      try {
        ...
        setHistory((prev) => ({
          backendLatency: [
            ...prev.backendLatency,
            data.metrics.backendLatencyMs ?? 0,
          ].slice(-40),
          activeSessions: [
            ...prev.activeSessions,
            data.metrics.activeSessions ?? 0,
          ].slice(-40),
          nodeHeap: [...prev.nodeHeap, data.metrics.nodeHeapUsedMb ?? 0].slice(
            -40,
          ),
          sepoliaLatency: [
            ...prev.sepoliaLatency,
            data.metrics.sepoliaRpcLatencyMs ?? 0,
          ].slice(-40),
          celoLatency: [
            ...prev.celoLatency,
            data.metrics.celoRpcLatencyMs ?? 0,
          ].slice(-40),
        }));
      } catch {
          ...
          <div className="flex flex-col">
            ...
            >
              {typeof metric.value === "number"
                ...
                : "--"}
              <span className="text-[7px] text-[#777] ml-0.5">
                {metric.unit}
              </span>
            </span>
          </div>
