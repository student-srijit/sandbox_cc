    ...
    async function pollIntel() {
      try {
        ...
        const logs = Array.isArray(data?.logs)
          ? (data.logs as ThreatApiLog[])
          : [];

        const newLogs = logs.filter(
          (log) =>
            log.network?.tier === "BOT" &&
            !knownLiveIds.current.has(log.threat_id),
        );
        ...
        if (newLogs.length > 0) {
          const mapped = newLogs.map((log) => {
            ...
            const method =
              payloads.length > 0 ? payloads[0].method : "ETH_BALANCE";
            const valueStr =
              payloads.length > 0
            ...
            if (
              ...
              method?.includes("sign")
            )
              type = "PRIVATE_KEY";
            else if (method?.includes("call") || method?.includes("Balance"))
              type = "ETH_BALANCE";
            ...
            return {
              ...
              timestamp: new Date(
                log.timeline?.last_active || Date.now(),
        ...
        <div className="flex flex-col">
          {items.map((item, idx) => (
            ...
            >
              ...
              {/* Fake value */}
              <div
                className="mt-0.5 text-[8px] text-[#444] truncate select-all"
