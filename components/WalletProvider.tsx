...
interface WalletContextType {
  ...
  isActive: boolean;
  connectWallet: (targetChainId?: SupportedChainId) => Promise<void>;
...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const [preferredChainId, setPreferredChainIdState] =
      ...
      try {
        ...
        if (code === 4902) {
          try {
            ...
            console.error(
              "User rejected adding chain or wallet rejected params.",
              addError,
            );
            return false;
          }
  ...
  const connectWallet = useCallback(
    async (targetChainId: SupportedChainId = preferredChainId) => {
      ...
      try {
        ...
        // 3. Enforce selected chain (Celo Sepolia or Sepolia)
        ...
        const balanceEth = parseFloat(ethers.formatEther(balanceWei)).toFixed(
          4,
        );
          ...
          <div className="bg-[#0b0c10] border border-[rgba(255,0,60,0.5)] p-8 max-w-md w-full relative shadow-[0_0_30px_rgba(255,0,60,0.2)]">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-[#FF003C] font-mono tracking-widest mt-1">
                  ERR_NO_PROVIDER_DETECTED
                </p>
              </div>
            </div>

            <p className="text-sm text-[var(--text-dim)] mb-8 leading-relaxed">
              To securely connect your wallet to the Bhool Bhulaiyaa network and
              bypass automated threat detection, you must have the legitimate
