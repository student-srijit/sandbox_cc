...
export default function Topbar() {
    const { isActive, address, balance, chainId, connectWallet } = useWallet()
    ...
    return (
        <header className="topbar-glass w-full h-full flex items-center justify-between px-7 relative z-10">
            ...
            <nav className="hidden lg:flex items-center gap-2 mx-6">
                {navItems.map((item) => {
                    const isActiveRoute = pathname === item.href || (item.href !== '/' && pathname?.startsWith(item.href))
                    return (
                        <Link
                            key={item.href}
                            href={item.href}
                            className={`text-[10px] px-3 py-2 tracking-[0.2em] uppercase border transition-all ${isActiveRoute
            ...
            <div className="flex items-center gap-6">
                ...
                {!isActive ? (
                    <button
                        ...
                        className="text-[9px] text-[#00FF41] hover:text-black tracking-[0.2em] uppercase font-bold transition-all border border-[#00FF41] bg-[#00FF41]/10 hover:bg-[#00FF41] shadow-[0_0_10px_rgba(0,255,65,0.15)] px-3 py-1.5"
                    >
                        CONNECT
                    </button>
                ) : (
